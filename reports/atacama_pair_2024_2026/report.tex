...
for this event are only 62--69\% DC. We ran full-MT and deviatoric lune searches in two data
configurations to separate signal from artefact --- surface-wave-only, and (following the
principle that more independent data is better) \emph{joint} with the safe 35--80~s body
band added. Both give the same non-DC \emph{shape}:

\begin{table}[H]\centering\small
\begin{tabular}{lllll}
\toprule
data & best mechanism & CLVD $\gamma$ & isotropic $\delta$ & \%DC (deviatoric) \\
\midrule
surface waves only & 162/18/$-$81 & $-9.1^\circ$ & $+23.5^\circ$ & 66\% \\
joint (body 35--80 s $+$ SW) & 342/63/$-$81 & $\mathbf{-9.1^\circ}$ & $\mathbf{+15.6^\circ}$ & \textbf{66\%} \\
\bottomrule
\end{tabular}
\end{table}

The \textbf{CLVD ($\gamma=-9.1^\circ$, 66\% DC) is identical in both} and matches the USGS
percent-DC; it is the credible, physical non-DC signal --- a composite rupture whose subevents
rotate mechanism (Jia et~al.'s ``mechanism transition'') sums to a non-DC tensor. The
\textbf{isotropic component is not robust}: adding body waves shrank it from
$\delta=+23.5^\circ$ to $+15.6^\circ$, confirming it was largely 1-D-structure error absorbed
by the free isotropic axis (and the variance-based likelihood did not converge). The
defensible statement is $\sim$66\% DC with a modest $-9^\circ$ CLVD, no trustworthy isotropic
term --- and the joint best plane (342/63) is the steep one, consistent with the finite-fault
...
\textbf{Why no surface-wave fits for 2024, and can it be fixed?} They do not exist because the
source lies below WISP's surface-wave Green's-function bank, whose \texttt{low.in} descriptor
spans depths 4--126.5~km (dep\_min 4, dep\_step 2.5, 50 samples); the 127.3-km hypocentre is
just past the deepest sample, so synthetics cannot be computed for \emph{any} plane through it.
We investigated extending the bank and found it structurally infeasible: (i) \texttt{fd\_bank}
is not generated by WISP but \emph{downloaded} as a global data product (Zenodo record
7236739), and the upstream generation code and velocity model are not vendored, so a homemade
deeper bank cannot be validated against the shipped one (risking silently-wrong Green's
functions); and (ii) the inversion binary hard-codes exactly 50 depth samples
(\texttt{real :: grid\_depth(50)} in \texttt{retrieve\_surf\_gf.f95}), so extending it also
requires recompiling WISP's Fortran. The body-only control (\S\ref{sec:planetest2024},
Part~III) shows this changes no result: WISP's surface waves constrain a moment the moment
tensor already fixes, not the finite slip. The moment tensor \emph{is} surface-wave-anchored
(Table~\ref{tab:mt2024}) --- MTUQ's syngine Green's functions are valid to 700~km --- so the
long-period moment is fully pinned by data WISP cannot use. The absence is also symmetric
between the two planes, which is what makes the body-only comparison fair by construction.

\section{Resolution: band, first pulses, and smoothness}
\label{sec:resolution2024}
Three questions about the body-wave fit quality, each settled by a direct test.

\textbf{Frequency band.} WISP band-passes the teleseismic P/SH at 0.006--1.0~Hz and inverts
8 dyadic wavelet scales, fitting scales 2--8 and zero-weighting only scale~1 ($>$1.25~Hz,
above the filter); the effective band is $\sim$0.01--1~Hz. Jia et~al.\ used 0.02--0.2~Hz for
teleseismic data --- our inversion already reaches to \emph{higher} frequency, not lower. The
band is physics-limited: the low corner passes periods longer than the $\sim$40-s source (to
capture the full moment); the high corner is bounded by 1-D Green's-function validity for
teleseismic P ($\sim$0.2--0.5~Hz, above which 3-D structure dominates) and by the subfault
Nyquist (8-km subfaults at $\sim$4~km/s cannot radiate coherent detail above $\sim$0.5~Hz).

\textbf{First pulses.} The under-fit first P pulses at some stations are a \emph{near-nodal
radiation} effect, identical to the 2026 case: correlating each station's first-pulse misfit
with its P-radiation coefficient $|F_P|$ gives $r=-0.55$; the ten worst-fit onsets all have
$|F_P|<0.2$ (BOSA, LBTB, SUR, PAB, TSUM, CMLA, TAM, FDFM, BBGH, MACI). Near a P node the
modelled first motion is $\sim$0, so the sharp observed onset is under-predicted while the
later, non-nodal P group fits well (Figure~\ref{fig:resolution}a). This is information (the
near-nodal discriminant), not a defect.

\textbf{Smoothness.} The model is as rough as the data allow, not over-regularised.
(i) 96\% of the observed P energy is below 0.2~Hz and is well fit (median variance reduction
$+0.51$); the $\sim$4\% above 0.2~Hz is essentially unfittable ($+0.02$), being incoherent 3-D
path effects (Figure~\ref{fig:resolution}b). (ii) Cutting the slip- and time-smoothness
penalties $5\times$ barely changes the misfit (0.1127$\to$0.1090) and does not recover the high
band ($+0.02\to+0.04$), but inflates slip roughness by $\sim$50\% --- unconstrained roughness
that fits noise. Jia et~al.\ resolved fine complexity from \emph{near-field strong motion} plus
back-projection, not from teleseismic high frequencies; our teleseismic-only model is correctly
the smooth, robust envelope of that rupture (same moment, duration, depth extent, plane).

\begin{figure}[H]\centering
\includegraphics[width=\textwidth]{fig_resolution_2024.png}
\caption{2024 resolution diagnostics. (a) First-pulse misfit vs.\ P-radiation coefficient:
the unfit onsets are near-nodal ($r=-0.55$). (b) Per-station variance reduction below
0.2~Hz (green; Jia's teleseismic band) vs.\ 0.2--1.0~Hz (red): the low band fits, the high
band does not.}
...
\section{Adding regional strong motion: can 1-D modelling work at these depths?}
\label{sec:regional}
Following Jia et~al.'s use of near-field data, we added regional strong-motion stations
(networks C/C1/CX/IU, acquired with \texttt{ffm manage acquire -t strong}) and re-ran WISP
jointly (\texttt{-t body -t strong}) for both events. The motivating caveat: regional 1-D
Green's-function modelling \emph{totally fails} for shallow strike-slip ruptures (as for the
2026 Venezuela doublet), where near-surface paths run through heterogeneous crust, basins and
strong scattering. Would it succeed for a \emph{deep intraslab} source, whose rays leave
steeply and traverse the near-1-D mantle? Two facts make the test clean: WISP computes the
near-field Green's functions on the fly (no depth cap, unlike the surface-wave bank,
\S\ref{sec:planetest2024}), and the strong-motion band (0.01--0.125~Hz) matches the low-frequency
regime where 1-D GFs are usable.

\textbf{It works --- unlike the shallow case.} Both events give a clearly positive
strong-motion variance reduction (2024: median $+0.36$; 2026: $+0.21$), capturing the
low-frequency near-field pulse and its timing (the high-frequency wiggles are not fit --- the
1-D-GF limit of \S\ref{sec:resolution2024}). This is a concrete, physically expected contrast
with the shallow Venezuela failure.

\textbf{Why 2026 keeps more stations than the larger 2024 event.} Counter-intuitively the
smaller M6.9 uses ten near-field stations and the larger M7.4 only three --- and it is not a
matter of signal usability (the closer M7.4 stations in fact fit slightly better). It is pure
geometry (Figure~\ref{fig:regmap}): the strong-motion network (CX/IPOC) is concentrated along
the coastal forearc, and the 2026 epicentre sits $\sim$80~km \emph{west}, closer to that
cluster, so ten stations fall inside WISP's 2$^\circ$ near-field radius. The 2024 epicentre lies
inland toward the Argentine border, shifting its 2$^\circ$ circle east so that most coastal
stations fall just outside it. The magnitude is irrelevant; epicentre--network proximity sets
the count.

\begin{figure}[H]\centering
\includegraphics[width=0.82\textwidth]{fig_regional_stations_map.png}
\caption{Regional strong-motion stations used in the joint inversions. Dashed circles are
WISP's 2$^\circ$ near-field selection radius around each epicentre (star). The forearc-clustered
network places ten stations inside the 2026 circle but only three inside the more inland 2024
circle. AF01/PB09/PB15 (both symbols) serve both events.}
\label{fig:regmap}
\end{figure}

\textbf{The near-field payoff scales with depth.} The 2026 event (109~km) keeps 10 near-field
stations within WISP's $\sim$2$^\circ$ cutoff and the joint slip model \emph{sharpens}
markedly --- the diffuse teleseismic asperity ($\sim$35~km down-dip, 0.70~m peak) collapses to
a compact patch at the hypocentre ($\sim$25~km down-dip, 108--113~km, 0.98~m peak;
Figure~\ref{fig:reg2026}). The 2024 event (127~km) keeps only 3 (a deeper source has fewer
stations in the near-field cone above it); its joint slip merely \emph{confirms} the
teleseismic model (centroid 138$\to$134~km, same geometry). Pushing the 2024 cutoff to
4.5$^\circ$ (16 stations) does not help but \emph{hurts} --- median VR $+0.36\to+0.12$ ---
because stations at 2--4.5$^\circ$ cannot be 1-D-modelled for a deep source and drag the
inversion. The default near-field selection is therefore physically correct, and the deep-source
near-field constraint is inherently limited by geometry.

\textbf{A plane tie-breaker for 2026.} The regional data fit the \emph{shallow} plane better
(strong-motion VR $+0.21$ shallow vs.\ $+0.08$ steep), independently reinforcing the weak
shallow preference from the teleseismic fit and the Tarapac\'a-2005 precedent --- the first
data-driven tie-breaker for the otherwise-ambiguous 2026 plane.

\textbf{Optimising the fit: shift-matching.} WISP's automatic pipeline aligns strong-motion
traces on theoretical arrivals only; cross-correlation shift-matching
(\texttt{ffm process shift-match}) removes the residual 1-D-velocity timing error and improves
the fit where it matters --- the 2026 median strong-motion variance reduction rises from $+0.21$
to $+0.31$ (fraction of channels above 0.3: 33\%\,$\to$\,53\%). The gain is real but bounded:
the vertical and radial main pulses then align well, while the transverse (SH) components stay
imperfect --- radiation-pattern sensitivity and the 1-D-Green's-function amplitude limit, not
timing. (For the 2024 event shift-matching is neutral, $+0.36\to+0.34$: its three stations are
so close that their timing was already correct.)

\textbf{Regional point-source MT is not reliable here (a negative result worth stating).}
Regional data can also, in principle, be inverted for the point-source moment tensor with MTUQ
(the CAP method's native regime). We did so for both events (24 regional broadband stations
each, syngine ak135 Green's functions). The result is a cautionary one: the regional
mechanisms land 32$^\circ$ (2026) and 71$^\circ$ (2024) in Kagan angle from the
well-constrained teleseismic solutions, with inflated magnitudes (\Mw~7.05, 7.10). The cause is
geometric --- every available station sits in the forearc, leaving a \textbf{$\sim$145$^\circ$
azimuthal gap} over the ocean and back-arc, which a point-source inversion cannot tolerate.
The lesson is consistent with the finite-fault result: regional data usefully constrains
rupture \emph{geometry} (a differential measurement, robust to the gap and to 1-D-GF errors ---
it sharpens the slip model), but \emph{not} the absolute point-source mechanism, which needs the
azimuthal coverage only teleseismic distances provide. The teleseismic moment tensor remains
authoritative; regional data is a complement for finite-fault detail, not a substitute.

\begin{figure}[H]\centering
\includegraphics[width=\textwidth]{fig_regional_2026_compare.png}
\caption{2026 slip model, teleseismic-only (left) vs.\ body\,$+$\,regional (right, 10
near-field stations): the near-field geometry localises the rupture into a compact asperity.}
\label{fig:reg2026}
\end{figure}
